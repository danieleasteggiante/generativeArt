function ParticleBlack(x,y, colorPixel,scl, cols, rows, ball_factor, scale, treshold_val, alpha_channel, jump_distance) {
  this.scale= scale
  
  this.ball_area_size = [15*this.scale, 10*this.scale, 8*this.scale, 5*this.scale, 3*this.scale]
  ...
  this.maxspeed = 1;
  //this.image = image;
  this.treshold_gradient = [0,20,30,40,60,treshold_val-20]
  this.treshold_gradient_active = undefined
  this.circle_opacity = [90,70,60,50,40,30]
  this.circle_size = [2*this.scale,1.8*this.scale,1.5*this.scale,1*this.scale,0.8*this.scale]
  this.alsoused=[];
  this.count= 0;
  this.scl=scl;
  this.cols=cols;
  ...
  this.incOpacity=0.0
  this.incRadius=0.0
  this.palette = [[255,0,255],[255,255,0],[50,255,180]]
  this.choice = random (0,60)
  if(this.choice <50){
      this.colour = this.palette[0]

  } else {
          this.colour = this.palette[floor(random(1,2.5))]
  ...
  this.setTarget = function(){
    ...
    for(let i=0; i < colorPixel.length; i++){
      if(colorPixel[i] < this.treshold_gradient[5]){
  ...
  this.checkNeightboors = function(xT,yT){
    ...
    if(colU <= this.treshold_gradient[5]){
      possibleMoves.push([x, y-excursion]);
    }
    if( colR <= this.treshold_gradient[5]){
      possibleMoves.push([x+excursion, y]);
    }
    if(colD <= this.treshold_gradient[5]){
      possibleMoves.push([x, y+excursion]);
    }
    if( colL <= this.treshold_gradient[5]){
      possibleMoves.push([x-excursion, y]);
    }
  ...
  this.createball = function(){
    ...
    let casuality = random(0,10000)
    
    if (this.ball_active==false){
      
      if (casuality>9997){
        this.ball_active = true
        this.circle_size = [floor(random(20)),floor(random(15)),floor(random(12)),floor(random(10)),floor(random(5))]
      }
      
    }
    
    
    if(this.ball_active == true){
      for(let i = 0; i< this.circle_size.length; i++){
        this.circle_size[i] = this.circle_size[i] - dec

      if(this.circle_size[i] <= this.circle_size_tmp[i]){
        this.circle_size = this.circle_size_tmp
        this.ball_active = false
        break
      }
      }
  ...
  this.treshold_activate =  function(){
    ...
    for(let i = this.treshold_gradient.length-1; i >= 0; i--){
  ...
  this.seek = function(){
    ...
    let noiseX = noise(this.count);
    let noiseY = noise(this.count + 1000);
  ...
  translate(this.location.x, this.location.y);
    let o = noise(this.incOpacity) * this.circle_opacity[this.treshold_gradient_active] + 500; 
    let d = noise(this.incRadius) * this.circle_size[this.treshold_gradient_active];
  if(this.jump == false ){
    
    fill(this.palette[1][0],this.palette[1][1],this.palette[1][2],alpha_channel[index_loc]);   
    circle(0,0,d);
    this.incRadius=this.incRadius + 0.04;
    this.incOpacity = this.incOpacity + 0.04;

  } else {
    if((this.treshold_gradient_active >= 4) && (this.choice > 15)){
    fill(this.palette[1][0],this.palette[1][1],this.palette[1][2],o);   
      circle(0,0,d);
      this.incRadius=this.incRadius + 0.04;
      this.incOpacity = this.incOpacity + 0.04;
  ...
  {
    let index = floor(this.target.x/this.scl) + floor(this.target.y/this.scl) * this.cols;
    colorPixel[index] = this.colorPixelTmp[index];
